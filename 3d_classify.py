...
def cnn_model():
    model = Sequential()
    inp = layers.Input(shape=(bands, channels, sampling_points, 1))

    conv1 = layers.Conv3D(32, kernel_size=(5, 5, 5), activation='relu', padding='same', input_shape=(bands, channels, sampling_points, 1))(inp)   #maybe relu
    #x22 = layers.Dropout(0.5)(x1)
    conv1 = layers.MaxPooling3D((2, 1, 2), padding='valid')(conv1)
...
for train_index, val_index in kfold.split(features, labels):
    ...
    x_train = scaler.fit_transform(X_train.reshape(len(X_train) * bands * sampling_points, channels)).reshape(X_train.shape)
    x_val = scaler.transform(X_val.reshape(len(X_val) * bands * sampling_points, channels)).reshape(X_val.shape)
    #x_test = scaler.transform(X_test.reshape(len(X_test) * time * sampling_points, 62)).reshape(X_test.shape)
